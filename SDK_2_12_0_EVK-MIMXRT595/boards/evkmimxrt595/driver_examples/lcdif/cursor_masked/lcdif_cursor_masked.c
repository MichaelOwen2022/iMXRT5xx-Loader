 ...
 * Readme:
 *
 * In this example, the screen is devided into two parts: red and blue. A cursor
...
typedef struct _lcdif_cursor_masked_row
{
    uint32_t colorRowLow;
    uint32_t colorRowHigh;
} lcdif_cursor_masked_row_t;
...
AT_NONCACHEABLE_SECTION_ALIGN(static lcdif_cursor_masked_row_t s_cursorBuffer[LCDIF_CURSOR_SIZE], DEMO_FB_ALIGN);
...
void DEMO_FillCursorBuffer(void)
{
    uint16_t i;

    /* Show background color set by LCDIF_SetCursorColor, it is black. */
    for (i = 0; i < LCDIF_CURSOR_SIZE / 4; i++)
    {
        s_cursorBuffer[i].colorRowLow  = 0x00000000;
        s_cursorBuffer[i].colorRowHigh = 0x00000000;
    }

    /* Show the backgroud frame buffer color. */
    for (; i < LCDIF_CURSOR_SIZE / 2; i++)
    {
        s_cursorBuffer[i].colorRowLow  = 0xFFFFFFFF;
        s_cursorBuffer[i].colorRowHigh = 0x00000000;
    }

    /* Show the foreground color set by LCDIF_SetCursorColor, it is white. */
    for (; i < LCDIF_CURSOR_SIZE * 3 / 4; i++)
    {
        s_cursorBuffer[i].colorRowLow  = 0x00000000;
        s_cursorBuffer[i].colorRowHigh = 0xFFFFFFFF;
    }

    /* Show the invertion of backgroud frame buffer color. */
    for (; i < LCDIF_CURSOR_SIZE; i++)
    {
        s_cursorBuffer[i].colorRowLow  = 0xFFFFFFFF;
        s_cursorBuffer[i].colorRowHigh = 0xFFFFFFFF;
    }
...
void DEMO_LCDIF_CursorMask(void)
{
    uint16_t x = 0;
    uint16_t y = 0;
    ...
    LCDIF_SetCursorColor(DEMO_LCDIF, LCDIF_MAKE_CURSOR_COLOR(0, 0, 0), LCDIF_MAKE_CURSOR_COLOR(0xFF, 0xFF, 0xFF));
