    ...
    if (mouseInstance->deviceState != mouseInstance->prevState)
    {
        ...
        switch (mouseInstance->deviceState)
        {
            ...
            case kStatus_DEV_Detached: /* device is detached */
                ...
                mouseInstance->L1sleepResumeState        = kStatus_Idle;
                mouseInstance->deviceSupportRemoteWakeup = 0U;
                mouseInstance->L1SetRemoteWakeup         = 0U;
                mouseInstance->L1sleepBus                = 0U;
            ...
            for (interfaceIndex = 0; interfaceIndex < configuration->interfaceCount; ++interfaceIndex)
            {
                ...
                else
                {
                    if (g_HostHidMouse.deviceState == kStatus_DEV_Idle)
                    {
                        ...
                        g_HostHidMouse.mouseBosHeadBuffer = s_MouseBosHeadBuffer;
                        ...
                        {
                            g_HostHidMouse.deviceSupportRemoteWakeup = 1U;
                        }
                        else
                        {
                            g_HostHidMouse.deviceSupportRemoteWakeup = 0U;
                        }
