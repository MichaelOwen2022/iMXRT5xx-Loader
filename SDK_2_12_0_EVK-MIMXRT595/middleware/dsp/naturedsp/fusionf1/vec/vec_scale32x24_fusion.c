...
)
{
  ...
  ae_int32x2  vx0w, vx1w, vy0w, vy1w, vcw;
  ...
  for (n=0; n<(N>>2); n++)
  {
    AE_LA32X2_IP(vx0w, x_align, px);
    AE_LA32X2_IP(vx1w, x_align, px);
    vx0f = (vx0w);
    ...
    vy1w = (vy1f);
    AE_SA32X2_IP(vy0w, y_align, py);
    AE_SA32X2_IP(vy1w, y_align, py);
  }
  n = N&3;
  if (N&2)
  {
    AE_LA32X2_IP(vx0w, x_align, px);
    vy0f = AE_MULFP32X2RAS(vx0w, vcf);
    AE_SA32X2_IP(vy0f, y_align, py);
  }
  AE_SA64POS_FP(y_align, py);
  if (N&1)
  {
    AE_L32_IP(vx0w, castxcc(const ae_int32, px), 0);
    vy0f = AE_MULFP32X2RAS(vx0w, vcf);
