              ...
              int N)
{
  ...
  { 
    ...
    XT_LASX2IP(vyf0, y_align, py);

    zf0 = XT_ADD_LHH_S(vxf0, vyf0);
    zf1 = XT_ADD_LLL_S(vxf0, vyf0);
    XT_SSIP(zf0, pz, 4);
    XT_SSIP(zf1, pz, 4);
  }
  if (N&1)
  {
    XT_LSIP(xf0, (const xtfloat *)px, 4);
    XT_LSIP(yf0, (const xtfloat *)py, 4);
    zf0 = XT_ADD_S(xf0, yf0);
    XT_SSIP(zf0, pz, 4);
  }
#else
  int n;
  ...
  { 
    ...
    vzf0 = XT_ADDX2_S(vxf0, vyf0);
    XT_pair_xtfloat_aligning_store_post_update_positive(vzf0, z_align, castxcc( pair_xtfloat, pz));
  }
