...
*/
/*
    Complex multiplication, 32 -bits precision, output is swapped (re in low part)
*/

#if 0 //__MNSD__
inline_ void _cmult32x32_swp(ae_int32x2 *result, ae_int32x2 *x, ae_int32x2 *y)
{
...
//  ae_f16x4    vF2;
  ae_int32x2  tw; 
  const ae_int32x2  * restrict ptw = (const ae_int32x2  *)twiddleSplit32;
...
//    vT = AE_CVT16X4(vB1, vB1);
  ...
  //  vF1 = (vA0);
   ...
   // vF0 = AE_MULFC32X16RAS_H(vF1, vF2);
    ...
    //ae_int32x2 test;
    tw = AE_INTSWAP(tw); 
    _cmult32x32(&vB0, &vA0, &tw);
    /*
    if (vB0._[0] != 0 && vB0._[1] != 0)
    {
        printf("\n"); 
    }
    */
              ...
              int scalingOpt)
{
  ...
  scale=fft_cplx32x32(y, x, h, scalingOpt);    /* fft of half-size              */
