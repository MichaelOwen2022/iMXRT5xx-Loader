...
#define IFFT_BUTTERFLY_S3(_T,step)         \
    {                                      \
...
static int ifft_core_s3(int16_t *y, int16_t *x, const tFftDescr* pDescr)
{
  ...
  while (stride > 8) 
  {
    ...
    do
    {
      ...
      IFFT_BUTTERFLY_S3(I,8)
      IFFT_BUTTERFLY_S3(X,step)
    } while(--lc);
  ...
  if (stride==8) 
  {
    ...
    vT0s = AE_L16X4_I(((const ae_int16x4*)ifft_twd_r8), 0);
    vT1s = AE_L16X4_I(((const ae_int16x4*)ifft_twd_r8), 8);
    vT2s = AE_L16X4_I(((const ae_int16x4*)ifft_twd_r8), 16);
...
#if (XCHAL_HAVE_FUSION_16BIT_BASEBAND)
#define IDFT4XI2(x0, x1, x2, x3)/* output x0, x3, x1, x2*/\
{\
    ae_int16x4 t1, t2, t3;\
...
static void stage_first_IDFT4XI2_inpl(int16_t *x, int16_t *twd, int N)
{
...
} //stage_first_IDFT4XI2_inpl
static void stage_last_IDFT4XI2_inpl(int16_t *x, int16_t *y, int N)
{
    ...
    for (i = 0; i<(N / 4); i += 2)
    {
        ...
        AE_S16X4RNG_X(h2, y13, ibitrev);
        AE_S16X4RNG_X(h3, y12, ibitrev);

        ibitrev = AE_ADDBRBA32(ibitrev, ibitrev_off);
    }

} //stage_last_IDFT4XI2_inpl
static void stage_inner_IDFT4XI2_inpl_merged(int16_t *x, int16_t *y, const int16_t *twd, int N, int stride)
{
    const int shift = 2;
    ae_int16x4 * restrict px = (ae_int16x4 *)x;
...
} //stage_inner_IDFT4XI2_inpl_merged
static void stage_inner_IDFT4XI2_inpl_ref(int16_t *x, int16_t *y, const int16_t *twd, int N, int stride)
{
    const int shift = 2;
    ae_int16x4 * restrict px = (ae_int16x4 *)x;
    ae_int16x4 * restrict py = (ae_int16x4 *)y;
...
#if !(XCHAL_HAVE_FUSION_16BIT_BASEBAND)
{
  ...
  scale += ifft_core_s3 (y, x, (const tFftDescr*)h);
...
#else
{
    ...
    stride = N; // The stride is quartered with every iteration of the outer loop.
    {
        ...
        stage_first_IDFT4XI2_inpl(x, (int16_t*)p_twd, N);
        stride >>= 2;
        if (stride > 4)
        {
            p_twd = (int16_t*)*p++;
            stage_inner_IDFT4XI2_inpl_merged(x, x, p_twd, N, stride);
            stride >>= 2;
            while (stride > 4)
            {
                p_twd = (int16_t*)*p++;
                stage_inner_IDFT4XI2_inpl_ref(x, x, p_twd, N, stride);
                stride >>= 2;
            }
        }
        if (stride == 4)
        {
            stage_last_IDFT4XI2_inpl(x, y, N);
        }
        else
        {
