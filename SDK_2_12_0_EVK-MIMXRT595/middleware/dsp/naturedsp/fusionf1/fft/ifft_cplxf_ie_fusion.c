...
DISCARD_FUN(int, ifft_cplxf_ie, (complex_float* y, complex_float* x, const complex_float* twd, int twdstep, int N))
#else
...
int ifft_cplxf_ie    (complex_float* y,complex_float* x, const complex_float* twd, int twdstep, int N)
{
  ...
  /*---------------------------------------------------*/
  /* Twiddle factors are conjugated to inverse the FFT */
  /*---------------------------------------------------*/
   ...
   * the last stage.                                                            */
  {
    ...
    for ( n=0; n<stride; n++ )
    {
      ...
      b2l = XT_MUL_S(a2l, t0h);
      b1h = XT_MUL_S(a1h, t1h);
      b1l = XT_MUL_S(a1l, t1h);
      b3h = XT_MUL_S(a3h, t2h);
      b3l = XT_MUL_S(a3l, t2h);

      XT_MADD_S(b2h, a2l, t0l);
      XT_MSUB_S(b2l, a2h, t0l);
      XT_MADD_S(b1h, a1l, t1l);
      XT_MSUB_S(b1l, a1h, t1l);
      XT_MADD_S(b3h, a3l, t2l);
      XT_MSUB_S(b3l, a3h, t2l);
  ...
  { 
    ...
    p3_ld = p2_ld+1;
    p0_st = (xtfloat *)(y);
    p1_st = (xtfloat *)(p0_st+2*N4);
    p2_st = (xtfloat *)(p1_st+2*N4);
    p3_st = (xtfloat *)(p2_st+2*N4);
    ...
    for ( n=0; n<N4; n++ )
    {
        ...
        a0h=XT_ADD_S(b0h , b1h);  a0l=XT_ADD_S(b0l , b1l);
        a0h=XT_MUL_S(a0h,recipN); a0l=XT_MUL_S(a0l,recipN);
        XT_SSX(a0l,p0_st,sizeof(float32_t)+idx);
        XT_SSX(a0h,p0_st,                  idx);
        a2h=XT_SUB_S(b0h , b1h) ; a2l=XT_SUB_S(b0l , b1l);
        a2h=XT_MUL_S(a2h,recipN); a2l=XT_MUL_S(a2l,recipN);
        XT_SSX(a2l,p2_st,sizeof(float32_t)+idx);
        XT_SSX(a2h,p2_st,                  idx);
        /* a1 <- (b2 + j*b3)*(1/N) */
        /* a3 <- (b2 - j*b3)*(1/N) */
        a1h=XT_SUB_S(b2h , b3l) ; a1l=XT_ADD_S(b2l , b3h);
        a1h=XT_MUL_S(a1h,recipN); a1l=XT_MUL_S(a1l,recipN);
        XT_SSX(a1l,p1_st,sizeof(float32_t)+idx);
        XT_SSX(a1h,p1_st,                  idx);
        a3h=XT_ADD_S(b2h , b3l) ; a3l=XT_SUB_S(b2l , b3h);
        a3h=XT_MUL_S(a3h,recipN); a3l=XT_MUL_S(a3l,recipN);
