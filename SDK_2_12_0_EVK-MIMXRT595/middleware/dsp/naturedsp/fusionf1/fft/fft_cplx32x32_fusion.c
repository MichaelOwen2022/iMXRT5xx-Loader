...
#if 0
    int32_t re = L_sub_ll(L_mpy_ll(x->_[0], y->_[0]), L_mpy_ll(x->_[1], y->_[1]));
    int32_t im = L_add_ll(L_mpy_ll(x->_[0], y->_[1]), L_mpy_ll(x->_[1], y->_[0]));
    result->_[0] = re; 
    result->_[1] = im; 
#else
#if 0
...
static void stage_inner_DFT4(int32_t *x, int32_t *y, const int32_t *twd, int N, int stride_bytes, int shift)
{
    ...
    for (j = 0; j < (N>>2); j++)
    {
        ...
        _cmult32x32(&x1, &x1, &t1);
        _cmult32x32(&x2, &x2, &t2);
        _cmult32x32(&x3, &x3, &t3);
...
#else
void stage_inner_DFT4(int32_t *x, int32_t *y, const int32_t *twd, int twstep, int N, int stride_bytes, int shift)
{
    ...
    int M = (N*sizeof(complex_fract32) / (4*stride_bytes));
    int i;
    int j;
    ...
    for (j = 0; j < M; j++)
    {
        ...
        px = (ae_int32x2 *)(j * stride_bytes * 4 + (uintptr_t)x);
        py = (ae_int32x2 *)(j * stride_bytes * 4 + (uintptr_t)y);
        ptw = (ae_int32x2 *)twd;
        ...
        do
        {
            ...
            AE_L32X2_XP(t1, ptw, 8);
            AE_L32X2_XP(t2, ptw, 8);
            AE_L32X2_XP(t3, ptw, twstep*24-16 );
...
void stage_inner_DFT4(int32_t *x, int32_t *y, const int32_t *twd, int twstep, int N, int stride_bytes, int shift)
{
    ...
    for (j = 0; j < M; j++)
    {
        ...
        px0 = (ae_int32x2 *)(j * stride_bytes * 4 + (uintptr_t)x);
        px1 = (ae_int32x2 *)(1 * stride_bytes + (uintptr_t)px0);
        px2 = (ae_int32x2 *)(2 * stride_bytes + (uintptr_t)px0);
        px3 = (ae_int32x2 *)(3 * stride_bytes + (uintptr_t)px0);

        py0 = (ae_int32x2 *)(j * stride_bytes * 4 + (uintptr_t)y);
        py1 = (ae_int32x2 *)(1 * stride_bytes +   (uintptr_t)py0);
        py2 = (ae_int32x2 *)(2 * stride_bytes +   (uintptr_t)py0);
        py3 = (ae_int32x2 *)(3 * stride_bytes +   (uintptr_t)py0);
