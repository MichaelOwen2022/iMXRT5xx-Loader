...
void cmtx_inv4x4f(complex_float* x)
{
    ...
    for(k=0; k<4; k++)
    {
        ...
        for (n=0; n<8; n++)
        {
            ...
            re = XT_HIGH_S(A2);    im = XT_LOW_S(A2);
            XT_MSUB_LHH_S(re, Ak, C2);    XT_MADD_LLL_S(re, Ak, C2);
            XT_MSUB_LLH_S(im, Ak, C2);    XT_MSUB_LHL_S(im, Ak, C2);
            XT_SSXC(re, castxcc(xtfloat, pA2), SZ_F32);
            XT_SSXC(im, castxcc(xtfloat, pA2), SZ_F32);
        }
    }
