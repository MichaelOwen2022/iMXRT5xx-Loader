...
/*
  NatureDSP Signal Processing Library. FIR part
    Real data linear convolution, 16x16-bit
*/
                        ...
                        int N, int M)
{
    ...
    ASSERT((int8_t *)s_ptr - (int8_t *)s <=
        (int)FIR_LCONVOLA16X16_SCRATCH_SIZE(N, M));
    ...
    //----------------------------------------------------------------------------
    // Copy reverted y[M] data into the aligned buffer and append 6 zeros:
    S = (const ae_int16x4 *)(y + M - 1);
    D = (ae_int16x4 *)y_buf;
    S_va = AE_LA64_PP(S);
    for (m = 0; m<((M + 3) >> 2); m++)
    {
        AE_LA16X4_RIP(y0, S_va, S);
        AE_S16X4_IP(y0, D, +8);
    }
