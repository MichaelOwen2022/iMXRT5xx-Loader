...
size_t firinterp16x16_alloc( int D, int M )
{
  NASSERT( D > 1 && M > 0 );
  NASSERT( M%4==0);
  return M*D*sizeof(int16_t) + M*sizeof(int32_t) + sizeof(firinterp16x16_t) + 7;
} // firinterp16x16_alloc()

/* Initialize the interpolator structure. The delay line is zeroed. */
firinterpf_handle_t firinterp16x16_init( void * objmem, 
                                             int D, int M, 
                                             const int16_t * restrict h )
{
    firinterp16x16_t* firinterp;
    void *           ptr;
    int32_t * restrict pd;
    int16_t * restrict ph;
    int m,d;
    ...
    pd = (int32_t*)(ph+M*D);
    firinterp=(firinterp16x16_t*)(pd+M);
 ...
 * response. */
void firinterp16x16_process( firinterpf_handle_t _firinterp, 
                             int16_t * restrict      y,
                       const int16_t * restrict      x, int N )
{
    ...
    NASSERT(_firinterp);
    state=(firinterp16x16_t *)_firinterp;
    NASSERT(x);
    NASSERT(y);
    NASSERT(state);
    ...
    for (n = 0; n<N; n += 4)
    {
        ...
        for (j = 0; j<D; j++)
        {
            ...
            q3 = AE_SLAS64S(AE_MULF48Q32SP16S_L( AE_ROUNDSQ32F48ASYM(q3), g_frac ));
            {
                ...
                XX = AE_ROUND16X4F32SASYM(t0,t0); AE_S16_0_XP(XX,pY,sizeof(int16_t)*D      );
                XX = AE_ROUND16X4F32SASYM(t1,t1); AE_S16_0_XP(XX,pY,sizeof(int16_t)*D      );
                XX = AE_ROUND16X4F32SASYM(t2,t2); AE_S16_0_XP(XX,pY,sizeof(int16_t)*D      );
                XX = AE_ROUND16X4F32SASYM(t3,t3); AE_S16_0_XP(XX,pY,sizeof(int16_t)*(1-3*D));
            }
        }
        /* update delay line */
