...
#if !(XCHAL_HAVE_FUSION_16BIT_BASEBAND)
{
    ...
    for ( n=0; n<N; n+=4)
    {
        ...
        AE_LA16X4POS_PC(ax1,pX1);
        AE_LA16X4POS_PC(ax2,pX2);
        AE_LA16X4POS_PC(ax3,pX3);
        pY =(const ae_int16x4*)y;
        __Pragma("loop_count min=1")
        for ( m=0; m<(M>>2); m++)
        {
            AE_L16X4_XC (X0123,pX0,sizeof(ae_int16x4));
            AE_LA16X4_IC(X1234,ax1,pX1);
            AE_LA16X4_IC(X2345,ax2,pX2);
            AE_LA16X4_IC(X3456,ax3,pX3);
