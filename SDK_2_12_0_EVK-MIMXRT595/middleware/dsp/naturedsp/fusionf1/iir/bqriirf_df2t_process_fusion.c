...
#define xtfloatx2_aloadip(a,uu,ptr)  XT_LASX2IP(a,uu,ptr)
#define xtfloatx2_astoreip(a,uu,ptr) XT_SASX2IP(a,uu,ptr)
                ...
                const float32_t *            x, int N)
{
    ...
    const xtfloat  * restrict pa2;
    const xtfloat  * restrict pb0;
    const xtfloat  * restrict pb1;
    const xtfloat  * restrict pb2;
    ...
    NASSERT(state->cf);
    M=state->M;
    /* process number of input samples which are the multiple of 4 */
    ...
    for(m=0; m<M; m++)
    {
        ...
        XT_LSIP(dx,pDrd,sizeof(float32_t)); 
        XT_LSIP(dz,pDrd,sizeof(float32_t)); 
        XT_LSIP(dw,pDrd,sizeof(float32_t)); 
        ...
        for (n=0;n<(N>>2);n++)
        {
            xtfloat out;
            out = dx;
            XT_LSIP(dx,pX,sizeof(x[0]));
            XT_SSIP(out,pZ,sizeof(x[0]));
            t=dz;
            XT_MADD_S(t,b0,dx);
            XT_MADD_S(dw,b1,dx);
            XT_MSUB_S(dw,a1,t ); 
        ...
        XT_SSIP(dx,pDwr,sizeof(float32_t));
        XT_SSIP(dz,pDwr,sizeof(float32_t));
        XT_SSIP(dw,pDwr,sizeof(float32_t));
        pX=(const xtfloat*)z;    /* switch pointer to the input data to the pointer to the output */
    }
    /* process last 2 samples if any */
    if(N&2)
    {
        ...
        for(m=0; m<M; m++)
        {
            ...
            XT_SSIP(dz,pDwr,sizeof(float32_t));
            XT_SSIP(dw,pDwr,sizeof(float32_t));
            pX=(const xtfloat*)(z+(N&~3));    /* switch pointer to the input data to the pointer to the output */
        }
    }
    // final scaling
    {
