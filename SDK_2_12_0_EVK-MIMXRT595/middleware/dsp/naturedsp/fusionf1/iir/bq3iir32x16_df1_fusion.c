...
typedef struct tag_bq3iir32x16_df1_t
{
...
} bq3iir32x16_df1_t, *bq3iir32x16_df1_ptr_t;
...
size_t bq3iir32x16_df1_alloc( int M )
{
  ...
  return ( ALIGNED_SIZE( sizeof(bq3iir32x16_df1_t), 4 )
                                               ...
                                               int16_t         gain )
{
  ...
  bq3iir    = (bq3iir32x16_df1_ptr_t)ALIGNED_ADDR( ptr, 4 );
  ptr       = bq3iir + 1;
  ...
  ASSERT( (int8_t*)ptr - (int8_t*)objmem <= (int)bq3iir32x16_df1_alloc(M ) );
...
} // bq3iir32x16_df1_init()

// Process data. The filter instance is identified with a handle returned by
// the initializing function.
void bq3iir32x16_df1( bq3iir32x16_df1_handle_t _bq3iir,
                      void    * restrict       s,
                      int32_t * restrict       r,
                const int32_t *                x, int N )
{
  bq3iir32x16_df1_ptr_t bq3iir = (bq3iir32x16_df1_ptr_t)_bq3iir;
  ...
  for ( l=0; l<3; l++ )
  {
    ...
    for ( m=0; m<M-1; m++ )
    {
      ...
      // Q31
      sx0sr0 = ae_f32x2_loadi( S, +0 );
      sx1sr1 = ae_f32x2_loadi( S, +8 );
      ...
      for ( n=0; n<(N>>1); n++ )
      {
        ...
        // s1 = g*x[n]; g*x[n+1]
        // Q31 <- Q31*Q15 - 15 w/ asym. rounding
        ...
        AE_S32_L_IP(y0, castxcc(ae_int32, R), 3 * sz_i32);
        AE_S32_L_IP(y1, castxcc(ae_int32, R), 3 * sz_i32);
      ...
      ae_f32x2_storeip( sx0sr0, S, +8 );
      ae_f32x2_storeip( sx1sr1, S, +8 );
    ...
    for ( n=0; n<(N>>1); n++ )
    {
      ...
      AE_S32RA64S_IP(q0, castxcc(ae_f32, R), 3 * sz_i32);
      AE_S32RA64S_IP(q1, castxcc(ae_f32, R), 3 * sz_i32);
