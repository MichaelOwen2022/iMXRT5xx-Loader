...
DISCARD_FUN(size_t, bq3iirf_df2_alloc, (int M))
DISCARD_FUN(bq3iirf_df2_handle_t, bq3iirf_df2_init, ( void * objmem, int M,
...
{
  NASSERT( M >= 0 );
  return sizeof(bq3iirf_df2_t)+(5+3*3)*M*sizeof(float32_t)+7;
} // bq3iirf_df2_alloc()
                                               ...
                                               int16_t         gain )
{
    ...
    cf = st+3*3*M;
    iir = (bq3iirf_df2_t*)(cf+5*M);
    iir->M=M;
    iir->st=st;
    iir->cf=cf;
    iir->gain=gain;
    /* clean up delay elements and copy coefficients */
    for(m=0; m<3*3*M; m++) st[m]=0.f;
    for(m=0; m<M; m++) 
    {
