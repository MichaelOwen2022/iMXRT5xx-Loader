...
#include "common.h"
#include "bq3iirf_df1.h"

#if !(XCHAL_HAVE_FUSION_FP)
DISCARD_FUN(void, bq3iirf_df1,( bq3iirf_df1_handle_t _bq3iir,float32_t * z,const float32_t * x, int N))
#else

// Process data. The filter instance is identified with a handle returned by
// the initializing function.
void bq3iirf_df1( bq3iirf_df1_handle_t _bq3iir,
                      float32_t * restrict       z,
                const float32_t *            x, int N)
{
    bq3iirf_df1_t * restrict state;
    ...
    for (l=0; l<3; l++)
    {
        ...
        pDrd=( const xtfloat*)(state->st+l*4*M);
        pDwr=(       xtfloat*)(state->st+l*4*M);
        ...
        for (m=0; m<M; m++)
        {
            ...
            XT_LSIP(dx1,pDrd,sizeof(float32_t)); 
            ...
            /* loop is unrolled 2 times:
               process by 4 samples per iteration */
            for (n=0;n<(N>>2);n++)
            {
                XT_LSXP(t,pX,4*3*(int)sizeof(float32_t));
                dz=XT_MUL_S (b2,dx1);
                XT_MADD_S(dz,b1,dx0);
                XT_MSUB_S(dz,a2,dy1);
                XT_MADD_S(dz,b0,t  );
                ...
                dx1=dx0;
                dx0=t;
                XT_SSXP(dz,pZ,4*3*(int)sizeof(float32_t));

                t=XT_LSX(pX,-3*3*(int)sizeof(float32_t));
                dz=XT_MUL_S (b2,dx1);
                XT_MADD_S(dz,b1,dx0);
                XT_MSUB_S(dz,a2,dy1);
                XT_MADD_S(dz,b0,t  );
                ...
                dx1=dx0;
                dx0=t;
                XT_SSX(dz,pZ,-3*3*(int)sizeof(float32_t));
                
                t=XT_LSI(pX,-2*3*(int)sizeof(float32_t));
                dz=XT_MUL_S (b2,dx1);
                XT_MADD_S(dz,b1,dx0);
                XT_MSUB_S(dz,a2,dy1);
                XT_MADD_S(dz,b0,t  );
                ...
                dx1=dx0;
                dx0=t;
                XT_SSI(dz,pZ,-2*3*(int)sizeof(float32_t));
                
                t=XT_LSI(pX,-1*3*(int)sizeof(float32_t));
                dz=XT_MUL_S (b2,dx1);
                XT_MADD_S(dz,b1,dx0);
                XT_MSUB_S(dz,a2,dy1);
                XT_MADD_S(dz,b0,t  );
                ...
                dx1=dx0;
                dx0=t;
                XT_SSI(dz,pZ,-1*3*(int)sizeof(float32_t));
            }
            /* process last (N%4) samples */
            if (N & 2)
            {
                XT_LSIP(t,pX,3*sizeof(float32_t));
                dz=XT_MUL_S (b2,dx1);
                XT_MADD_S(dz,b1,dx0);
                XT_MSUB_S(dz,a2,dy1);
                XT_MADD_S(dz,b0,t  );
                ...
                dx1=dx0;
                dx0=t;
                XT_SSIP(dz,pZ,3*sizeof(float32_t));
            ...
            XT_SSIP(dx1,pDwr,sizeof(float32_t));
