...
typedef struct tag_bq3iir16x16_df2_t
{
...
} bq3iir16x16_df2_t, *bq3iir16x16_df2_ptr_t;
                                               ...
                                               int16_t         gain )
{
  ...
  bqriir    = (bq3iir16x16_df2_ptr_t)ALIGNED_ADDR( ptr, 4 );
  ptr       = bqriir + 1;
  ...
  ASSERT( (int8_t*)ptr - (int8_t*)objmem <= (int)bq3iir16x16_df2_alloc(M ) );
...
{
  bq3iir16x16_df2_ptr_t bqriir = (bq3iir16x16_df2_ptr_t)_bqriir;
  ...
  for ( l=0; l<3; l++ )
  {
    ...
    for ( m=0; m<M-1; m++ )
    {
      ...
      // Load input sample.
      AE_L16_IP(inx, castxcc(const ae_int16,X), 3*sz_i16);
      // d0 = g*x[n] - a1*d1 - a2*d2
      // Q30 <- Q15*Q14 + 1
      ...
      for ( n=0; n<N-2; n++ )
      {
        ...
        // Store output sample.
        // Q15
        AE_S16_0_IP(outy,castxcc( ae_int16,R), 3*sz_i16);
      }
