                ...
                const complex_float *                x, int N)
{
    ...
    for(reim=0; reim<2; reim++)
    {
        ...
        for (m=0; m<M; m++)
        {
            ...
            for (n=0;n<(N>>1);n++)
            {
                XT_LSIP(t,pX,2*sizeof(float32_t));
                dz=XT_MUL_S (b2,dx1);
                XT_MADD_S(dz,b1,dx0);
                XT_MSUB_S(dz,a2,dy1);
                XT_MADD_S(dz,b0,t  );
                ...
                dx1=dx0;
                dx0=t;
                XT_SSIP(dz,pZ,2*sizeof(float32_t));
            ...
            pX=((const xtfloat*)z)+reim;    /* switch pointer to the input data to the pointer to the output */
        }
    }
    // final scaling
    {
        ...
        for (n=0;n<2*N;n++)
        {
            ((float32_t*)z)[n]=((float32_t*)z)[n]*(float32_t)ft;
        }
    }
}
#endif
