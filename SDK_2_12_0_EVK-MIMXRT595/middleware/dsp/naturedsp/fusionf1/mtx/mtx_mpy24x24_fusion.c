...

{
  ...
  int         K, offa, offb;
  ae_int32x2  vpw;
  ae_f24x2    vxl, val, vbl, vcl, vdl, zl;
  ae_f24 *    Z1;
  ae_f24 *    Z2;
  ae_f24 *    T1;
  ae_f24 *    T2;
      ...
      for(n=0; n<(N>>1); ++n)
        {
          ...
          T1=(ae_f24 *)(offa)+p;
          T2=(ae_f24 *)(offb)+p;
          AE_L32F24_IP(val,T1, 4);
          AE_L32F24_XP(vbl,T1, 4);
          AE_L32F24_IP(vcl,T2, 4);
          AE_L32F24_XP(vdl,T2, 4);
          a0 = AE_SELP24_LL(val,vcl);
          b0 = AE_SELP24_LL(vbl,vdl);
          AE_S32X2F24_IP(a0,castxcc(ae_f24x2,Z1),8);
          AE_S32X2F24_IP(b0,castxcc(ae_f24x2,Z2),8);
        }
        if (N&1) 
        { 
          ...
          T1=(ae_f24 *)(offa)+p;
          AE_L32F24_IP(val,T1, 4);
          AE_L32F24_IP(vbl,T1, 0);
          AE_S32F24_L_IP(val, castxcc(ae_f24, Z1), 4);
          AE_S32F24_L_IP(vbl, castxcc(ae_f24, Z2), 4);
          AE_S32F24_L_IP(zl,castxcc(ae_f24,Z1),0);
          AE_S32F24_L_IP(zl, castxcc(ae_f24, Z2), 0);
        }
        for(m=0; m<M; m++)
        {
          ae_f64 vah,vbh;
          ...
          px = (const ae_f24x2 *)x;
          px = (ae_f24x2 *)((ae_f24 *) px + m*N);
          x_align = AE_LA64_PP(px);
          for(n=0; n<(K>>1); ++n)
          {
            AE_LA32X2F24_IP(vxl, x_align, px); 
            AE_L32X2F24_IP(val, castxcc(ae_f24x2,Z1), 8); 
            AE_L32X2F24_IP(vbl, castxcc(ae_f24x2,Z2), 8);
            AE_MULAAFD24_HH_LL(vah,val,vxl);
            AE_MULAAFD24_HH_LL(vbh,vbl,vxl);
          }
          ...
          vy2 = AE_ROUNDSP24Q48ASYM(vbh);
          AE_L32_IP(vpw, pz, 4); 
          offb = AE_MOVAD32_H(vpw);
          Y1=(ae_f24 *)(offb)+p;
          AE_S32F24_L_IP(vy1,Y1,4);
          if (p+1<P) AE_S32F24_L_IP(vy2,Y1,4);
        }
    }
} /* mtx_mpy24x24() */
