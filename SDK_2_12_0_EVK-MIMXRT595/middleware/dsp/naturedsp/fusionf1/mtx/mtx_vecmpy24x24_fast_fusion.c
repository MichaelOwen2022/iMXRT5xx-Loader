...

{
  ...
  const ae_f24x2      *  restrict px2 = (const ae_f24x2     *)x;
  
  int m,n;
  ae_int32x2  zz;
  ae_f24x2    vxl,val,vyl;
  ...
  for(m=M; m>0; m-=2)
  {
    ...
    for(n=(N>>1); n>0; n--)
    {
      ...
      AE_L32X2F24_IP(val, py, 8); 
      AE_MULAAFD24_HH_LL(vah,vxl,val);
      AE_MULAAFD24_HH_LL(vbh,vyl,val);
    }
    vha = (vah);
    vhb = (vbh);
    ...
    vah = (vha);
    vbh = (vhb);
    vy1 = AE_ROUND24F48SASYM(vah);
    AE_S32F24_L_IP(vy1,pz,4);
    vy1 = AE_ROUND24F48SASYM(vbh);
    AE_S32F24_L_IP(vy1,pz,4);
  }
