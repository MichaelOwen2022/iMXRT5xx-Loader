...
DISCARD_FUN(void, cmtx_det2x2f, (complex_float   * restrict d, const complex_float   *restrict x, int L))
#else
/*-------------------------------------------------------------------------
...
void cmtx_det2x2f(complex_float   * restrict d, const complex_float   *restrict x, int L)
{
  ...
  for (l = 0; l<(L); l++)
  {
    ...
    A0 = XT_MUL_LHH_S(x0, x3);
    XT_MSUB_LLL_S(A0, x0, x3);
    XT_MSUB_LHH_S(A0, x1, x2);
    XT_MADD_LLL_S(A0, x1, x2);

    A1 = XT_MUL_LHL_S(x0, x3);
    XT_MADD_LLH_S(A1, x0, x3);
    XT_MSUB_LHL_S(A1, x1, x2);
    XT_MSUB_LLH_S(A1, x1, x2);

    XT_SSIP(A0, castxcc(float32_t, pz), 4);
    XT_SSIP(A1, castxcc(float32_t, pz), 4);
  }
} /* cmtx_det2x2f() */
#endif
