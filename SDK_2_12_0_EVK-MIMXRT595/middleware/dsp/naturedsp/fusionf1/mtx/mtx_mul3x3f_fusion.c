...
void mtx_mul3x3f(float32_t * restrict z, const float32_t *restrict x, const float32_t *restrict y, int L)
{
  ...
  int l;
  xtfloatx2 vx0, vy0, vx1, vx2, vy1, vy2, vy3, vy4;
  xtfloat A0, A1, A2;
  ...
  for (l = 0; l<L; l++)
  {
    ...
    XT_MADD_LLL_S(A0, vx0, vy1);
    XT_MADD_LHH_S(A0, vx1, vy3);

    XT_MADD_LLH_S(A1, vx0, vy2);
    XT_MADD_LHL_S(A1, vx1, vy3);

    XT_MADD_LLL_S(A2, vx0, vy2);
    XT_MADD_LHH_S(A2, vx1, vy4);

    XT_SSIP(A0, pz, 4);
    XT_SSIP(A1, pz, 4);
    XT_SSIP(A2, pz, 4);

    A0 = XT_MUL_LLH_S(vx1, vy0);
    XT_MADD_LHL_S(A0, vx2, vy1);
    XT_MADD_LLH_S(A0, vx2, vy3);

    A1 = XT_MUL_LLL_S(vx1, vy0);
    XT_MADD_LHH_S(A1, vx2, vy2);
    XT_MADD_LLL_S(A1, vx2, vy3);

    A2 = XT_MUL_LLH_S(vx1, vy1);
    XT_MADD_LHL_S(A2, vx2, vy2);
    XT_MADD_LLH_S(A2, vx2, vy4);
  ...
  for (l = 0; l<L; l++,y+=9)
  {
    ...
    XT_LSIP(x2, castxcc(xtfloat, px), 4 * 7);
    vx0 = (xtfloatx2)x0;
    vx1 = (xtfloatx2)x1;
    vx2 = (xtfloatx2)x2;
    XT_LASX2IP(vy0, y_align, py);
    ...
    A1 = XT_MUL_LHL_S(vx0, vy0);
    A2 = XT_MUL_LHH_S(vx0, vy1);
    XT_MADD_LLL_S(A0, vx1, vy1);
    XT_MADD_LHH_S(A0, vx2, vy3);

    XT_MADD_LLH_S(A1, vx1, vy2);
    XT_MADD_LHL_S(A1, vx2, vy3);

    XT_MADD_LLL_S(A2, vx1, vy2);
    XT_MADD_LHH_S(A2, vx2, vy4);
