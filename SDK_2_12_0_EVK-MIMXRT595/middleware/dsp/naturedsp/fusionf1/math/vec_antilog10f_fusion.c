...
void vec_antilog10f (float32_t * restrict y, const float32_t* restrict x, int N)
{
  ...
  */
#if ANTILOG_ALG==1
  const xtfloat * restrict X = (xtfloat*)x;
  ...
  static const uint32_t TAB[2] = { 0x6a4d3c26, 0x7fc00000 };
  pT = (xtfloat  *)TAB;
  XT_LSIP(x0, X, sz_f32);
  XT_LSIP(x1, X, sz_f32);
  ...
  t0 = AE_MOVINT32X2_FROMINT32(XT_TRUNC_S(x0, 25));
  t1 = AE_MOVINT32X2_FROMINT32(XT_TRUNC_S(x1, 25));
  for (n = 0; n<(N >> 1); n++)
  {
    ae_f24x2 f0;
    ae_f32x2 acc0;
  ...
  if (N&1)
  {
    ...
    d0 = AE_MOVINT32X2_FROMINT32(AE_L32_I((ae_int32 *)pT, 0)); 
    wh = AE_MUL32_LL(t0, d0);
    wh0 = AE_SRAI64(wh, 22);
    e0 = AE_TRUNCI32X2F64S(wh0, wh0, 0);
    wh1 = AE_SLLI64(wh0, 32);
    u0 = AE_TRUNCI32X2F64S(wh1, wh1, 0);
    u0 = AE_SRLI32(u0, 1);
