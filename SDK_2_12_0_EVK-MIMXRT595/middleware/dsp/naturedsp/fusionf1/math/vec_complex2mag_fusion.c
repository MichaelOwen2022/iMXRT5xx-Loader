...
void       vec_complex2mag (float32_t  * restrict y, const complex_float  * restrict x, int N)
{
  ...
  */
  ...
  float32_t ALIGN(8) scr[blkSize*3];
  ...
  for (blkIx = 0; blkIx<blkNum; blkIx++)
  {
    ...
    X = (xtfloatx2*)((uintptr_t)x + blkIx*blkSize*2*sz_f32);
    S_wr = (xtfloatx2*)scr;
    /*
    * x_re = fabsf( crealf(x) );
    * x_im = fabsf( cimagf(x) );
    ...
    */
    ...
    for (n = 0; n<((blkLen + 1) / 2); n++)
    {
      ...
      y0 = XT_AE_MOVXTFLOATX2_FROMINT32X2(nsa);
    ...
    for (n = 0; n<(blkLen + 1) / 2; n++)
    {
      ...
      XT_LSIP(xf1, castxcc(const xtfloat, S_rd), 3 * sz_f32);

      xf0 = XT_SQRT_S(xf0);
      xf1 = XT_SQRT_S(xf1);
      XT_SSIP(xf0, castxcc(xtfloat, S_wr), sz_f32);
      XT_SSIP(xf1, castxcc(xtfloat, S_wr), 3 * sz_f32);
    }
    __Pragma("no_reorder");
    S_rd = (xtfloatx2*)(scr);
    __Pragma("loop_count min=1");
    ...
    for (n = 0; n<(blkLen + 1) / 2; n++)
    {
      ...
      nsa = XT_AE_MOVINT32X2_FROMXTFLOATX2(y0);
      XT_LSIP(xf0, castxcc(const xtfloat, S_rd), sz_f32);
      ...
      xf0 = XT_MUL_S(xf0, XT_HIGH_S(x0));
      xf1 = XT_MUL_S(xf1, XT_LOW_S(x0));
  ...
  if (N&1)
  {
    ...
    XT_SSIP(xf, castxcc(xtfloat, Y), sz_f32);
  }
} /* vec_complex2mag() */
#endif
