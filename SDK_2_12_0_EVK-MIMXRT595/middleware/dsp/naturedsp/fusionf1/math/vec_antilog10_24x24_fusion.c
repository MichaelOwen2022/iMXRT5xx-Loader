...
void vec_antilog10_24x24(int32_t * restrict y, const int32_t* restrict x, int N)
{
    int n;
    ...
    {
        ae_f24x2 xf24;
        AE_LA32X2F24_IP(xf24, x_align, px); X=AE_MOVINT32X2_FROMF24X2(xf24);
        al=AE_MUL32_LL(1783446566,X);
        ah=AE_MUL32_LH(1783446566,X);
        X=AE_TRUNCI32X2F64S(ah,al,11);
        E=AE_SRAI32(X,25);
        E=AE_SUB32(E,15);
    ...
    if(N&1)
    {
        X=AE_L32F24_I((const ae_f24*)px, 0);
        al=AE_MUL32_LL(X,1783446566);
        X=AE_TRUNCI32X2F64S(al,al,11);
        E=AE_SRAI32(X,25);
        E=AE_SUB32(E,15);
        X=AE_AND32(X,AE_MOVDA32X2(0x1ffffff,0x1ffffff));
        X=AE_SUB32(X,AE_MOVDA32X2(0x2000000,0x2000000));
