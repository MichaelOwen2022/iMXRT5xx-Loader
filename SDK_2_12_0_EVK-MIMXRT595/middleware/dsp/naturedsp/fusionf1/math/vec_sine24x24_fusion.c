...
void vec_sine24x24 (  f24 * restrict y, const f24 * restrict x, int N)
#if SIN24X24_ALG==0
{
  /*
...
*/
  ...
  if (N&1)
  {
   ...
   // vaw = AE_AND32(vaw, vww);
    ...
    vaw = AE_AND32(vaw, vkw);
    off = AE_MOVAD32_H(vaw);
    vcs = AE_L32_X((const ae_int32 *)sine_table32, off); //cos vxa
    vsf = (vsn);//sin
    vdf = (vdl);//dx
    vxf = (vcs); //cos
    vdf = (vdl);//delta
    AE_MULAFP32X2RAS(vsf, vxf, vdf); //y = sin(x0)+cos(x0)dx
    vrw = (vsf);
...
#elif SIN24X24_ALG==1
{
    ...
    for (n=0; n<(N>>1);n++)
    {
        AE_LA32X2F24_IP(t, x_align,px); X=t;
    ...
    if(N&1)
    {
        t=AE_L32F24_I((const ae_f24*)px, 0); X=t;
        sx=AE_LT32(X,AE_ZERO32());
        X=AE_SLLI32(X,9);
        X=AE_SRLI32(X,8);
        X=AE_SUB32(_0x800000,AE_ABS32(AE_SUB32(X,_0x800000)));
