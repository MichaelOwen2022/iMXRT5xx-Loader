...
void vec_logn_24x24 (f24 * restrict y,const f24 * restrict x, int N)
{
  ...
  vmw = AE_MOVDA32X2(0x7fffff, 0x58B90BFC);/*ln(2) in Q.31*/
  ...
  for (n=0; n<N; ++n)
  {
    ...
    vxf = (vyw);
    vdf = (vmw);
    vxf = AE_MULFP32X2RAS(vxf, vdf);   /* ln(x) = log2(x)*ln(2) */
    vyw = (vxf);
    AE_MOVT32X2(vyw, viw, inf);
    AE_S32_L_IP(vyw, py, sizeof(*py));
  }
}
