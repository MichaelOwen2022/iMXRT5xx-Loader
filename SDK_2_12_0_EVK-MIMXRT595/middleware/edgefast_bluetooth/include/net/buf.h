...
struct net_buf_pool {
	...
	osa_msgq_handle_t free;
	OSA_MSGQ_HANDLE_DEFINE(freeHandle, CONFIG_BT_MSG_QUEUE_COUNT, sizeof(void *));
...
{
	struct net_buf_pool *pool = net_buf_pool_get((int)buf->pool_id);
    osa_status_t ret;

	ret = OSA_MsgQPut(pool->free, &buf);
