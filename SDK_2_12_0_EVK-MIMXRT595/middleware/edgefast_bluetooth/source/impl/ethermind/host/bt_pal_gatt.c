...
#define LOG_ENABLE IS_ENABLED(CONFIG_BT_DEBUG_GATT)
...
#define SC_TIMEOUT	BT_MSEC(10)
#define CCC_STORE_DELAY	BT_SECONDS(1)

#define DB_HASH_TIMEOUT	BT_MSEC(10)
...
STRUCT_SECTION_DEFINE(bt_gatt_service_static);
...
{
	struct gatt_cf_cfg *cfg;
	const uint8_t *value = (const uint8_t *)buf;
...
static uint8_t gen_hash_m(const struct bt_gatt_attr *attr, uint16_t handle,
			  void *user_data)
{
	struct gen_hash_state *state = (struct gen_hash_state *)user_data;
	...
	case 0x2905: /* GATT Characteristic Aggregated Format */
		...
		if (bt_aes_128_cmac_update(&state->state, (uint8_t *)&value,
				   sizeof(handle)) != 0) {
			state->err = -EINVAL;
        ...
        if (true == has_value)
        {
            ...
            if (bt_aes_128_cmac_update(&state->state, data, len) !=
                0) {
                state->err = -EINVAL;
                return BT_GATT_ITER_STOP;
            }
...
static void db_hash_gen(bool store)
{
	...
	if (bt_aes_128_cmac_setup(&state.state, key) != 0) {
	...
	if (bt_aes_128_cmac_final(db_hash, &state.state) != 0) {
...
static int bt_gatt_store_cf(struct bt_conn *conn)
{
	...
	(defined(CONFIG_BT_GATT_CLIENT) && ((CONFIG_BT_GATT_CLIENT) > 0U))
...
static uint8_t found_attr(const struct bt_gatt_attr *attr, uint16_t handle,
			  void *user_data)
{
	const struct bt_gatt_attr **found = (const struct bt_gatt_attr **)user_data;
...
static void gatt_insert(struct bt_gatt_service *svc, uint16_t last_handle)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&db, tmp, node, struct bt_gatt_service) {
...
static int gatt_register(struct bt_gatt_service *svc)
{
	...
	last = SYS_SLIST_PEEK_TAIL_CONTAINER(&db, last, node, struct bt_gatt_service);
...
#ifdef EDGEFAST_BT_HAVE_GATT_DB_DYNAMIC_GLOBAL_ARRAY
    /**
...
#else /* EDGEFAST_BT_HAVE_GATT_DB_DYNAMIC_GLOBAL_ARRAY */
    config_ptr = NULL;
#endif /* EDGEFAST_BT_HAVE_GATT_DB_DYNAMIC_GLOBAL_ARRAY */

    (void)BT_gatt_db_init(config_ptr);
...
#if (defined(CONFIG_BT_GATT_CLIENT) && ((CONFIG_BT_GATT_CLIENT) > 0U))
#if ((defined(CONFIG_BT_SETTINGS) && ((CONFIG_BT_SETTINGS) > 0U)) && (defined(CONFIG_BT_SMP) && ((CONFIG_BT_SMP) > 0U)))
...
#if (defined(CONFIG_BT_GATT_DYNAMIC_DB) && (CONFIG_BT_GATT_DYNAMIC_DB > 0U)) || \
    ((defined(CONFIG_BT_GATT_CACHING) && (CONFIG_BT_GATT_CACHING > 0U)) && (defined(CONFIG_BT_SETTINGS) && (CONFIG_BT_SETTINGS > 0U)))
				    ...
				    bt_gatt_attr_func_t func, void *user_data)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&db, svc, node, struct bt_gatt_service) {
		struct bt_gatt_service *next;

		next = SYS_SLIST_PEEK_NEXT_CONTAINER(svc, node, struct bt_gatt_service);
		if (next) {
...
#if (defined(CONFIG_BT_GATT_NOTIFY_MULTIPLE) && ((CONFIG_BT_GATT_NOTIFY_MULTIPLE) > 0U))
...
BT_WORK_DEFINE(nfy_mult_work, notify_mult_process);
...
#if (defined(CONFIG_BT_GATT_ENFORCE_CHANGE_UNAWARE) && ((CONFIG_BT_GATT_ENFORCE_CHANGE_UNAWARE) > 0U))
			  ...
			  const void *data, uint16_t length)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&sub->list, params, tmp, node, struct bt_gatt_subscribe_params) {
			       ...
			       uint16_t length)
{
	...
	while (buf.len > sizeof(*nfy)) {
		...
		SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&sub->list, params, tmp,
						  node, struct bt_gatt_subscribe_params) {
			if (handle != params->value_handle) {
				continue;
			}
...
#if !(defined(CONFIG_BT_GATT_READ_MULTIPLE) && (CONFIG_BT_GATT_READ_MULTIPLE > 0)) || !(defined(CONFIG_BT_EATT) && (CONFIG_BT_EATT > 0))
static int gatt_read_mult_vl(struct bt_conn *conn,
			     struct bt_gatt_read_params *params)
{
	return -ENOTSUP;
				      ...
				      void *user_data)
{
	...
	if (write != length) {
		BT_WARN("Unable to allocate length %u: only %lu written",
			length, write);
		net_buf_unref(buf);
		return -ENOMEM;
	}
...
#if (defined(CONFIG_BT_GATT_AUTO_DISCOVER_CCC) && ((CONFIG_BT_GATT_AUTO_DISCOVER_CCC) > 0U))
		      ...
		      struct bt_gatt_subscribe_params *params)
{
	...
	/* Lookup existing subscriptions */
	SYS_SLIST_FOR_EACH_CONTAINER(&sub->list, tmp, node, struct bt_gatt_subscribe_params) {
...
static void add_subscriptions(struct bt_conn *conn)
{
	...
	/* Lookup existing subscriptions */
	SYS_SLIST_FOR_EACH_CONTAINER(&sub->list, params, node, struct bt_gatt_subscribe_params) {
		if (bt_addr_le_is_bonded(conn->id, &conn->le.dst) &&
		    !atomic_test_bit(params->flags,
...
void bt_gatt_att_max_mtu_changed(struct bt_conn *conn, uint16_t tx, uint16_t rx)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&callback_list, cb, node, struct bt_gatt_cb) {
