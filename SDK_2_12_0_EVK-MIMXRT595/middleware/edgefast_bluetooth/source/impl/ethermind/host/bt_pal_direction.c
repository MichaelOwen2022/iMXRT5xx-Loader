...
#define LOG_ENABLE IS_ENABLED(CONFIG_BT_DEBUG_DF)
...
#if ((defined(CONFIG_BT_DF_CONNECTIONLESS_CTE_RX) && (CONFIG_BT_DF_CONNECTIONLESS_CTE_RX > 0U)) || (defined(CONFIG_BT_DF_CONNECTION_CTE_RX) && (CONFIG_BT_DF_CONNECTION_CTE_RX > 0U)))
				    ...
				    const struct bt_df_adv_cte_tx_param *params)
{
	...
	cp = (struct bt_hci_cp_le_set_cl_cte_tx_params *)net_buf_add(buf, sizeof(*cp));
	cp->handle = adv->handle;
	cp->cte_len = params->cte_len;
	cp->cte_type = get_hci_cte_type((enum bt_df_cte_type)params->cte_type);
	cp->cte_count = params->cte_count;

	if (params->num_ant_ids) {
		uint8_t *dest_ant_ids = (uint8_t *)net_buf_add(buf, params->num_ant_ids);
				...
				uint8_t *max_cte_len)
{
	...
	rp = (struct bt_hci_rp_le_read_ant_info *)rsp->data;
					...
					bool enable)
{
	...
	cp = (struct bt_hci_cp_le_set_cl_cte_tx_enable *)net_buf_add(buf, sizeof(*cp));
...
#if (defined(CONFIG_BT_DF_CONNECTION_CTE_TX) && (CONFIG_BT_DF_CONNECTION_CTE_TX > 0))
...
#if (defined(CONFIG_BT_DF_CONNECTION_CTE_RSP) && (CONFIG_BT_DF_CONNECTION_CTE_RSP > 0U))
...
#if (defined(CONFIG_BT_DF_CONNECTION_CTE_RSP) && (CONFIG_BT_DF_CONNECTION_CTE_RSP > 0))
