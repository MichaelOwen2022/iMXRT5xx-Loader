			      ...
			      const bt_addr_t *addr)
{
	...
	else
	{
		...
		cp = (struct bt_hci_cp_le_set_adv_set_random_addr *)net_buf_add(buf, sizeof(*cp));
...
static int hci_id_del(const bt_addr_le_t *addr)
{
	...
	cp = (struct bt_hci_cp_le_rem_dev_from_rl *)net_buf_add(buf, sizeof(*cp));
	bt_addr_le_copy(&cp->peer_id_addr, addr);
...
{
#if ((defined(CONFIG_BT_HCI_VS_EXT) && (CONFIG_BT_HCI_VS_EXT > 0U)) \
 || (defined(CONFIG_BT_CTLR) && (CONFIG_BT_CTLR > 0U)))
