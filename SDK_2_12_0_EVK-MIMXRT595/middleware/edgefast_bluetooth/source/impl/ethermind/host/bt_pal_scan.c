...
static sys_slist_t pa_sync_cbs;
...
static int set_le_ext_scan_enable(uint8_t enable, uint16_t duration)
{
	...
	cp = (struct bt_hci_cp_le_set_ext_scan_enable *)net_buf_add(buf, sizeof(*cp));
...
static int bt_le_scan_set_enable_legacy(uint8_t enable)
{
	...
	cp = (struct bt_hci_cp_le_set_scan_enable *)net_buf_add(buf, sizeof(*cp));
...
#if (defined(CONFIG_BT_ISO_BROADCAST) && (CONFIG_BT_ISO_BROADCAST > 0U))
...
void bt_hci_le_adv_report(struct net_buf *buf)
{
	...
	while (num_reports--) {
		...
		evt = (struct bt_hci_evt_le_advertising_info *)net_buf_pull_mem(buf, sizeof(*evt));
