...
#define SHELL_ADVANCE (0U)
#endif

#ifndef SHELL_COMMAND_CONNECTOR
#define SHELL_COMMAND_CONNECTOR '.'
#endif

/*! @brief Macro to bypass arguments check */
#if (defined(SHELL_ADVANCE) && (SHELL_ADVANCE > 0))
#define SHELL_IGNORE_PARAMETER_COUNT     (0xFFFE)
#define SHELL_IGNORE_PARAMETER_COUNT_MAX (0xFFFF)
#define SHELL_IGNORE_PARAMETER_COUNT_MIN (0xFFFF)
#else
...
#define SHELL_HANDLE_SIZE                                                                                      \
    (112 + 8 + SHELL_HISTORY_COUNT * SHELL_BUFFER_SIZE + SHELL_BUFFER_SIZE + SERIAL_MANAGER_READ_HANDLE_SIZE + \
     SERIAL_MANAGER_WRITE_HANDLE_SIZE)
#else
#define SHELL_HANDLE_SIZE                                                                                             \
    (112 + OSA_SEM_HANDLE_SIZE + OSA_TASK_HANDLE_SIZE + SHELL_HISTORY_COUNT * SHELL_BUFFER_SIZE + SHELL_BUFFER_SIZE + \
...
typedef struct _shell_command
{
    ...
    const cmd_function_t pFuncCallBack; /*!< A pointer to the callback function that
                                    returns the output generated by the command. */
#if (defined(SHELL_ADVANCE) && (SHELL_ADVANCE > 0))
    union
    {
        uint32_t cExpectedNumberOfParameters;
        struct
        {
            uint16_t min; /*!< Min expected parameter count, which may be zero. */
            uint16_t max; /*!< Max expected parameter count, which may be zero. */
        } range;
    } parameter;
#else
...
#if (defined(SHELL_ADVANCE) && (SHELL_ADVANCE > 0))
    struct _shell_command **commandList; /*!< Sub-command list entry */
#endif                                   /* SHELL_ADVANCE */
} shell_command_t;
 ...
 * SHELL_RegisterCommand(shell_handle, SHELL_COMMAND(exit));
 * @endcode
 *
...
#define SHELL_COMMAND_DEFINE(command, descriptor, callback, paramCount)                             \
    ...
    shell_command_t g_shellCommand##command = {                                                     \
        (#command), (char *)(descriptor), (callback), {((paramCount) | ((paramCount) << 16))}, {0}, \
    }
 ...
 * This is a example,
 * @code
 * SHELL_COMMAND_EXDEFINE(exit, NULL, "\r\n\"exit\": Exit program\r\n", SHELL_ExitCommand, 0, 0);
 * SHELL_RegisterCommand(shell_handle, SHELL_COMMAND(exit));
 * @endcode
 *
 * @param command The command string of the command. The double quotes do not need. Such as exit for "exit",
 *                help for "Help", read for "read".
 * @param cmdList Sub-set of the command.
 * @param descriptor The description of the command is used for showing the command usage when "help" is typing.
 * @param callback The callback of the command is used to handle the command line when the input command is matched.
 * @param miniArg Number of required parameters
 * @param addtionArg Number of optional parameters
 */
#define SHELL_COMMAND_EXDEFINE(command, cmdList, descriptor, callback, miniArg, addtionArg) \
                                                                                            \
    shell_command_t g_shellCommand##command = {                                             \
        .pcCommand           = (#command),                                                  \
        .pcHelpString        = (char *)(descriptor),                                        \
        .pFuncCallBack       = (callback),                                                  \
        .commandList         = (cmdList),                                                   \
        .parameter.range.min = (miniArg),                                                   \
        .parameter.range.max = (addtionArg + miniArg),                                      \
        .link                = {0},                                                         \
    }
/*!
 * @brief Defines the shell command structure
 ...
 * This is a example,
 * @code
 * shell_command_t g_shellSet[] =
 * {
 *     SHELL_COMMAND_EXDEFINE_ANONYMOUS(sub1, NULL, "\r\n\"sub1\": sub1 command\r\n", SHELL_Sub1Command, 0, 0),
 *     NULL
 * }
 * SHELL_COMMAND_EXDEFINE(main, g_shellSet, "\r\n\"main\": Main Command\r\n", SHELL_MainCommand, 0, 0);
 * SHELL_RegisterCommand(shell_handle, SHELL_COMMAND(main));
 * @endcode
 *
 * @param command The command string of the command. The double quotes do not need. Such as exit for "exit",
 *                help for "Help", read for "read".
 ...
 * @param addtionArg Number of optional parameters
 */
#define SHELL_COMMAND_EXDEFINE_ANONYMOUS(command, cmdList, descriptor, callback, miniArg, addtionArg) \
                                                                                                      \
    (&(shell_command_t){                                                                              \
        .pcCommand           = (#command),                                                            \
        .pcHelpString        = (char *)(descriptor),                                                  \
...
#define SHELL_COMMAND_DEFINE(command, descriptor, callback, paramCount)  \
                                                                         \
    shell_command_t g_shellCommand##command = {                          \
        (#command), (char *)(descriptor), (callback), (paramCount), {0}, \
    }
#endif
/*!
        ...
        extern "C"
{
     ...
     * This is an example.
     * @code
     *   static SHELL_HANDLE_DEFINE(shell_handle);
     *   SHELL_Init((shell_handle_t)shell_handle, (serial_handle_t)s_serialHandle, "Test@SHELL>");
     * @endcode
